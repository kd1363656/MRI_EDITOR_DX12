...
namespace FWK::Graphics
{
	...
	private:
		...
		ComPtr<IDXGISwapChain4> m_swapChain   = nullptr;	// ダブルバッファリングにおける描画バッファの切り替えに使用する

		ComPtr<ID3D12GraphicsCommandList7> m_commandList      = nullptr;	// 描画命令を出すために必要なインターフェース
		ComPtr<ID3D12CommandQueue>         m_commandQueue     = nullptr;	// コマンドアロケーターに蓄積されている命令を実行するインターフェース
		ComPtr<ID3D12CommandAllocator>     m_commandAllocator = nullptr;	// コマンドを蓄積していくインターフェース

		ComPtr<ID3D12Fence1> m_fence = nullptr;		// "CPU"と"GPU"間の同期をとるのに必要

		std::unique_ptr<FWK::Graphics::RTVDescriptorHeap> m_rtvDescriptorHeap = nullptr;		//レンダーターゲット用のビュー
